...
if (isset($_POST['proses'])) {
    ...
    if (empty($id_pegawai) || empty($nama_pegawai) || empty($jabatan)) {
        ...
        if ($conn) {
            $stmt = $conn->prepare("INSERT INTO pegawai (id_pegawai, nama_pegawai, jabatan) VALUES (?, ?, ?)");
            if ($stmt) {
                $stmt->bind_param("sss", $id_pegawai, $nama_pegawai, $jabatan);
                if ($stmt->execute()) {
                    header("Location: pegawai-lihat.php?success=1");
                    exit();
                } else {
...
<body>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Tambah Pegawai</h2>
        <span class="text-muted">15 Juni 2025, 12:44 AM WIB</span>
      </div>
      <img src="../images/kop.jpg" alt="Logo" height="40">
    </div>

    <div class="card">
      <h5 class="mb-4 text-center">Form Tambah Pegawai</h5>
      <?php if (isset($error)): ?>
      ...
      <form action="" method="post">
        <div class="form-group">
          ...
          <input type="text" name="id_pegawai" id="id_pegawai" class="form-control" placeholder="Masukkan ID Pegawai" value="<?php echo isset($_POST['id_pegawai']) ? htmlspecialchars($_POST['id_pegawai']) : ''; ?>">
        </div>
        <div class="form-group">
          <label for="nama_pegawai">Nama</label>
          <input type="text" name="nama_pegawai" id="nama_pegawai" class="form-control" placeholder="Masukkan Nama Pegawai" value="<?php echo isset($_POST['nama_pegawai']) ? htmlspecialchars($_POST['nama_pegawai']) : ''; ?>">
        </div>
        <div class="form-group">
          <label for="jabatan">Jabatan</label>
          <input type="text" name="jabatan" id="jabatan" class="form-control" placeholder="Masukkan Jabatan" value="<?php echo isset($_POST['jabatan']) ? htmlspecialchars($_POST['jabatan']) : ''; ?>">
        </div>
