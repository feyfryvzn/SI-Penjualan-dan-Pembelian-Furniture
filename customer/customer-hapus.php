...
// Ambil data customer untuk ditampilkan
...
if (isset($_POST['proses'])) {
    if ($conn) {
        ...
        if ($stmt) {
            $stmt->bind_param("s", $id_customer);
            if ($stmt->execute()) {
                header("Location: customer-lihat.php?success=delete");
                exit();
            } else {
...
<body>
  <div class="sidebar">
    ...
    <a href="../customer/customer-lihat.php" class="active"><i class="fas fa-users"></i><span>Customer</span></a>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Hapus Customer</h2>
        <small class="text-muted">14 Juni 2025, 11:46 PM WIB</small>
      </div>
      <img src="../images/kop.jpg" alt="Logo" height="40">
    </div>

    <div class="card">
      <h5 class="mb-4 text-center">Konfirmasi Hapus Customer</h5>
      <?php if (isset($error)): ?>
        <div class="alert alert-danger"><?php echo htmlspecialchars($error); ?></div>
      <?php endif; ?>
      <p>Apakah Anda yakin ingin menghapus customer berikut?</p>
      <div class="form-group">
        <label>ID Customer</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['id_customer']); ?>" readonly>
      </div>
      <div class="form-group">
        <label>Nama Customer</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['nama_customer']); ?>" readonly>
      </div>
      <div class="form-group">
        <label>Alamat</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['alamat']); ?>" readonly>
      </div>
      <div class="form-group">
        <label>No HP</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['no_hp']); ?>" readonly>
      </div>
      <form action="" method="post">
        <div class="text-right">
          <a href="customer-lihat.php" class="btn btn-secondary">Batal</a>
