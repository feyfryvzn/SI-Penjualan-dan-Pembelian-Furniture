...
if (isset($_POST['proses'])) {
    ...
    // Validasi input
    if (empty($nama_customer) || empty($alamat) || empty($no_hp)) {
        $error = "Semua kolom harus diisi!";
    } else {
        if ($conn) {
            $stmt = $conn->prepare("UPDATE customer SET nama_customer = ?, alamat = ?, no_hp = ? WHERE id_customer = ?");
            if ($stmt) {
                $stmt->bind_param("ssss", $nama_customer, $alamat, $no_hp, $id_customer);
                if ($stmt->execute()) {
                    header("Location: customer-lihat.php?success=update");
                    exit();
                } else {
...
<body>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Ubah Customer</h2>
        <small class="text-muted">14 Juni 2025, 11:43 PM WIB</small>
      </div>
      <img src="../images/kop.jpg" alt="Logo" height="40">
    </div>

    <div class="card">
      <h5 class="mb-4 text-center">Form Ubah Customer</h5>
      <?php if (isset($error)): ?>
      ...
      <form action="" method="post">
        <div class="form-group">
          ...
          <input type="text" name="id_customer" id="id_customer" class="form-control" value="<?php echo htmlspecialchars($data['id_customer']); ?>" readonly>
        </div>
        <div class="form-group">
          <label for="nama_customer">Nama Customer</label>
          <input type="text" name="nama_customer" id="nama_customer" class="form-control" placeholder="Masukkan Nama Lengkap" value="<?php echo htmlspecialchars($data['nama_customer']); ?>">
        </div>
        <div class="form-group">
          <label for="alamat">Alamat</label>
          <input type="text" name="alamat" id="alamat" class="form-control" placeholder="Masukkan Alamat" value="<?php echo htmlspecialchars($data['alamat']); ?>">
        </div>
        <div class="form-group">
          <label for="no_hp">No HP</label>
          <input type="text" name="no_hp" id="no_hp" class="form-control" placeholder="Masukkan Nomor Handphone" value="<?php echo htmlspecialchars($data['no_hp']); ?>">
        </div>
