...
if (isset($_POST['proses'])) {
    ...
    if (empty($id_kayu) || empty($jenis_hasil_hutan)) {
        $error = "Semua kolom harus diisi!";
    } else {
        if ($conn) {
            $stmt = $conn->prepare("INSERT INTO jenis_hasil_hutan (id_kayu, jenis_hasil_hutan) VALUES (?, ?)");
            if ($stmt) {
                $stmt->bind_param("ss", $id_kayu, $jenis_hasil_hutan);
                if ($stmt->execute()) {
                    header("Location: jenis_hasil_hutan-lihat.php?success=1");
                    exit();
                } else {
...
<head>
  ...
  <title>Jenis Hasil Hutan - Tambah</title>
...
<body>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Tambah Jenis Hasil Hutan</h2>
        <span class="text-muted">01:25 AM WIB, 15 Juni 2025</span>
      </div>
      <img src="../images/kop.jpg" alt="Logo" height="40">
    </div>

    <div class="card">
      <h5 class="mb-4 text-center">Form Tambah Jenis Hasil Hutan</h5>
      <?php if (isset($error)): ?>
      ...
      <form action="" method="post">
        <div class="form-group">
          ...
          <input type="text" name="id_kayu" id="id_kayu" class="form-control" placeholder="Masukkan ID Kayu" value="<?php echo isset($_POST['id_kayu']) ? htmlspecialchars($_POST['id_kayu']) : ''; ?>">
        </div>
        <div class="form-group">
          <label for="jenis_hasil_hutan">Jenis Hasil Hutan</label>
          <input type="text" name="jenis_hasil_hutan" id="jenis_hasil_hutan" class="form-control" placeholder="Masukkan Jenis Hasil Hutan" value="<?php echo isset($_POST['jenis_hasil_hutan']) ? htmlspecialchars($_POST['jenis_hasil_hutan']) : ''; ?>">
        </div>
