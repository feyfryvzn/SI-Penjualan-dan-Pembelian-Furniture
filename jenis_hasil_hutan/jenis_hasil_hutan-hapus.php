...
if (isset($_POST['proses'])) {
    if ($conn) {
        $stmt = $conn->prepare("DELETE FROM jenis_hasil_hutan WHERE id_kayu = ?");
        if ($stmt) {
            $stmt->bind_param("s", $id_kayu);
            if ($stmt->execute()) {
                header("Location: jenis_hasil_hutan-lihat.php?success=delete");
                exit();
            } else {
...
<head>
  ...
  <title>Jenis Hasil Hutan - Hapus</title>
...
<body>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Hapus Jenis Hasil Hutan</h2>
        <small class="text-muted">01:35 AM WIB, 15 Juni 2025</small>
      </div>
      <img src="../images/kop.jpg" alt="Logo" height="40">
    </div>

    <div class="card">
      <h5 class="mb-4 text-center">Konfirmasi Hapus Jenis Hasil Hutan</h5>
      <?php if (isset($error)): ?>
        <div class="alert alert-danger"><?php echo htmlspecialchars($error); ?></div>
      <?php endif; ?>
      <p>Apakah Anda yakin ingin menghapus jenis hasil hutan berikut?</p>
      <div class="form-group">
        <label>ID Kayu</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['id_kayu']); ?>" readonly>
      </div>
      <div class="form-group">
        <label>Jenis Hasil Hutan</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['jenis_hasil_hutan']); ?>" readonly>
      </div>
      <form action="" method="post">
        <div class="text-right">
          <a href="jenis_hasil_hutan-lihat.php" class="btn btn-secondary">Batal</a>
