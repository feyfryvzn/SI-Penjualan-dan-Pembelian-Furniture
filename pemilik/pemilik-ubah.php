...
if (!isset($_GET['id_pemilik']) || empty($_GET['id_pemilik'])) {
    header("Location: pemilik-lihat.php");
    exit();
}

$id_pemilik = $_GET['id_pemilik'];
$stmt = $conn->prepare("SELECT * FROM pemilik WHERE id_pemilik = ?");
...
if (isset($_POST['proses'])) {
    ...
    if (empty($nama_pemilik) || empty($alamat_pemilik) || empty($id_kelurahan)) {
        $error = "Semua kolom (kecuali ID) harus diisi!";
    } else {
        if ($conn) {
            $stmt = $conn->prepare("UPDATE pemilik SET nama_pemilik = ?, alamat_pemilik = ?, id_kelurahan = ? WHERE id_pemilik = ?");
            if ($stmt) {
                $stmt->bind_param("ssss", $nama_pemilik, $alamat_pemilik, $id_kelurahan, $id_pemilik);
                if ($stmt->execute()) {
                    header("Location: pemilik-lihat.php?success=update");
                    exit();
                } else {
...
<body>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Ubah Pemilik</h2>
        <small class="text-muted">01:50 AM WIB, 15 Juni 2025</small>
      </div>
      <img src="../images/kop.jpg" alt="Logo" height="40">
    </div>

    <div class="card">
      <h5 class="mb-4 text-center">Form Ubah Pemilik</h5>
      <?php if (isset($error)): ?>
      ...
      <form action="" method="post">
        <div class="form-group">
          ...
          <input type="text" name="id_pemilik" id="id_pemilik" class="form-control" value="<?php echo htmlspecialchars($data['id_pemilik']); ?>" readonly>
        </div>
        <div class="form-group">
          <label for="nama_pemilik">Nama Pemilik</label>
          <input type="text" name="nama_pemilik" id="nama_pemilik" class="form-control" value="<?php echo htmlspecialchars($data['nama_pemilik']); ?>">
        </div>
        <div class="form-group">
          <label for="alamat_pemilik">Alamat Pemilik</label>
          <input type="text" name="alamat_pemilik" id="alamat_pemilik" class="form-control" value="<?php echo htmlspecialchars($data['alamat_pemilik']); ?>">
        </div>
        <div class="form-group">
          ...
          <select name="id_kelurahan" id="id_kelurahan" class="form-control">
            ...
            while ($row = mysqli_fetch_assoc($kelurahan)) {
                $selected = ($row['id_kelurahan'] == $data['id_kelurahan']) ? 'selected' : '';
