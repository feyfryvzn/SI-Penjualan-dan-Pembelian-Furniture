...
if (isset($_POST['proses'])) {
    if ($conn) {
        ...
        if ($stmt) {
            $stmt->bind_param("s", $id_pemilik);
            if ($stmt->execute()) {
                header("Location: pemilik-lihat.php?success=delete");
                exit();
            } else {
...
<body>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Hapus Pemilik</h2>
        <small class="text-muted">01:55 AM WIB, 15 Juni 2025</small>
      </div>
      <img src="../images/kop.jpg" alt="Logo" height="40">
    </div>

    <div class="card">
      <h5 class="mb-4 text-center">Konfirmasi Hapus Pemilik</h5>
      <?php if (isset($error)): ?>
        <div class="alert alert-danger"><?php echo htmlspecialchars($error); ?></div>
      <?php endif; ?>
      <p>Apakah Anda yakin ingin menghapus pemilik berikut?</p>
      <div class="form-group">
        <label>ID Pemilik</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['id_pemilik']); ?>" readonly>
      </div>
      <div class="form-group">
        <label>Nama Pemilik</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['nama_pemilik']); ?>" readonly>
      </div>
      <div class="form-group">
        <label>Alamat Pemilik</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['alamat_pemilik']); ?>" readonly>
      </div>
      <form action="" method="post">
        <div class="text-right">
          <a href="pemilik-lihat.php" class="btn btn-secondary">Batal</a>
