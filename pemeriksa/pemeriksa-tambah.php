...
if (isset($_POST['proses'])) {
    ...
    // Validasi input
    if (empty($id_pemeriksa) || empty($nama_pemeriksa) || empty($no_registrasi) || empty($instansi)) {
        $error = "Semua kolom harus diisi!";
    } else {
        if ($conn) {
            $stmt = $conn->prepare("INSERT INTO pemeriksa (id_pemeriksa, nama_pemeriksa, no_registrasi, instansi) VALUES (?, ?, ?, ?)");
            if ($stmt) {
                $stmt->bind_param("ssss", $id_pemeriksa, $nama_pemeriksa, $no_registrasi, $instansi);
                if ($stmt->execute()) {
                    header("Location: pemeriksa-lihat.php?success=1");
                    exit();
                } else {
...
<body>
  <div class="sidebar">
    ...
    <a href="../pemeriksa/pemeriksa-lihat.php" class="active"><i class="fas fa-user-check"></i><span>Pemeriksa</span></a>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Tambah Pemeriksa</h2>
        <span class="text-muted">01:05 AM WIB, 15 Juni 2025</span>
      </div>
      <img src="../images/kop.jpg" alt="Logo" height="40">
    </div>

    <div class="card">
      <h5 class="mb-4 text-center">Form Tambah Pemeriksa</h5>
      <?php if (isset($error)): ?>
      ...
      <form action="" method="post">
        <div class="form-group">
          ...
          <input type="text" name="id_pemeriksa" id="id_pemeriksa" class="form-control" placeholder="Masukkan ID Pemeriksa" value="<?php echo isset($_POST['id_pemeriksa']) ? htmlspecialchars($_POST['id_pemeriksa']) : ''; ?>">
        </div>
        <div class="form-group">
          <label for="nama_pemeriksa">Nama</label>
          <input type="text" name="nama_pemeriksa" id="nama_pemeriksa" class="form-control" placeholder="Masukkan Nama Pemeriksa" value="<?php echo isset($_POST['nama_pemeriksa']) ? htmlspecialchars($_POST['nama_pemeriksa']) : ''; ?>">
        </div>
        <div class="form-group">
          <label for="no_registrasi">No Registrasi</label>
          <input type="text" name="no_registrasi" id="no_registrasi" class="form-control" placeholder="Masukkan No Registrasi" value="<?php echo isset($_POST['no_registrasi']) ? htmlspecialchars($_POST['no_registrasi']) : ''; ?>">
        </div>
        <div class="form-group">
          <label for="instansi">Instansi</label>
          <input type="text" name="instansi" id="instansi" class="form-control" placeholder="Masukkan Instansi" value="<?php echo isset($_POST['instansi']) ? htmlspecialchars($_POST['instansi']) : ''; ?>">
        </div>
