...
// Ambil data pemeriksa untuk ditampilkan
...
if (isset($_POST['proses'])) {
    if ($conn) {
        $stmt = $conn->prepare("DELETE FROM pemeriksa WHERE id_pemeriksa = ?");
        if ($stmt) {
            $stmt->bind_param("s", $id_pemeriksa);
            if ($stmt->execute()) {
                header("Location: pemeriksa-lihat.php?success=delete");
                exit();
            } else {
...
<body>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Hapus Pemeriksa</h2>
        <small class="text-muted">01:15 AM WIB, 15 Juni 2025</small>
      </div>
      <img src="../images/kop.jpg" alt="Logo" height="40">
    </div>

    <div class="card">
      <h5 class="mb-4 text-center">Konfirmasi Hapus Pemeriksa</h5>
      <?php if (isset($error)): ?>
        <div class="alert alert-danger"><?php echo htmlspecialchars($error); ?></div>
      <?php endif; ?>
      <p>Apakah Anda yakin ingin menghapus pemeriksa berikut?</p>
      <div class="form-group">
        <label>ID Pemeriksa</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['id_pemeriksa']); ?>" readonly>
      </div>
      <div class="form-group">
        <label>Nama Pemeriksa</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['nama_pemeriksa']); ?>" readonly>
      </div>
      <div class="form-group">
        <label>No Registrasi</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['no_registrasi']); ?>" readonly>
      </div>
      <div class="form-group">
        <label>Instansi</label>
        <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['instansi']); ?>" readonly>
      </div>
      <form action="" method="post">
        <div class="text-right">
          <a href="pemeriksa-lihat.php" class="btn btn-secondary">Batal</a>
