...
if (isset($_POST['proses'])) {
    ...
    // Validasi input
    if (empty($id_barang) || empty($nama_barang) || empty($harga_satuan)) {
        $error = "Semua kolom harus diisi!";
    } elseif ($harga_satuan < 0) {
        $error = "Harga satuan tidak boleh negatif!";
    } else {
        if ($conn) {
            $stmt = $conn->prepare("INSERT INTO barang (id_barang, nama_barang, harga_satuan) VALUES (?, ?, ?)");
            if ($stmt) {
                $stmt->bind_param("ssi", $id_barang, $nama_barang, $harga_satuan);
                if ($stmt->execute()) {
                    header("Location: barang-lihat.php?success=add");
                    exit();
                } else {
...
<body>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Tambah Barang</h2>
        <small class="text-muted">14 Juni 2025, 11:53 PM WIB</small>
      </div>
      <img src="../images/kop.jpg" alt="Logo" height="40">
    </div>

    <div class="card">
      <h5 class="mb-4 text-center">Form Tambah Barang</h5>
      <?php if (isset($error)): ?>
      ...
      <form action="" method="post">
        ...
        <div class="form-group">
          ...
          <input type="number" name="harga_satuan" id="harga_satuan" class="form-control" placeholder="Masukkan Harga Satuan" min="0">
        </div>
