...
if (isset($_POST['proses'])) {
    ...
    // Validasi input
    if (empty($no_nota) || empty($id_customer) || empty($id_pegawai) || empty($tanggal) || empty($DP)) {
        $error = "Semua kolom harus diisi!";
    } else {
        if ($conn) {
            // Gunakan prepared statement untuk insert awal
            $stmt = $conn->prepare("INSERT INTO penjualan (no_nota, id_customer, id_pegawai, tanggal, jumlah_Rp, DP, sisa) VALUES (?, ?, ?, ?, 0, ?, 0)");
            if ($stmt) {
                $stmt->bind_param("sssss", $no_nota, $id_customer, $id_pegawai, $tanggal, $DP);
                if ($stmt->execute()) {
                    // Hitung ulang jumlah_Rp dari detail_penjualan
                    $result = mysqli_query($conn, "SELECT SUM(jumlah) as total FROM detail_penjualan WHERE no_nota = '$no_nota'");
                    $row = mysqli_fetch_assoc($result);
                    $jumlah_Rp = $row['total'] ?? 0;
                    $sisa = $jumlah_Rp - $DP;

                    // Update jumlah_Rp dan sisa
                    $stmt_update = $conn->prepare("UPDATE penjualan SET jumlah_Rp = ?, sisa = ? WHERE no_nota = ?");
                    if ($stmt_update) {
                        $stmt_update->bind_param("dds", $jumlah_Rp, $sisa, $no_nota);
                        $stmt_update->execute();
                        $stmt_update->close();
                    }

                    header("Location: penjualan-lihat.php?success=1");
                    exit();
                } else {
...
<body>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Tambah Penjualan</h2>
        <small class="text-muted">01:40 AM WIB, 15 Juni 2025</small>
      </div>
      <img src="../images/kop.jpg" alt="Logo" height="40">
    </div>

    <div class="card">
      <h5 class="mb-4 text-center">Form Tambah Penjualan</h5>
      <?php if (isset($error)): ?>
      ...
      <form action="" method="post">
        <div class="form-group">
          ...
          <input type="text" name="no_nota" id="no_nota" class="form-control" placeholder="Masukkan No Nota" value="<?php echo isset($_POST['no_nota']) ? htmlspecialchars($_POST['no_nota']) : ''; ?>" required>
        </div>
        <div class="form-group">
          <label for="id_customer">Customer</label>
          <select name="id_customer" id="id_customer" class="form-control" required>
            <option value="" disabled <?php echo !isset($_POST['id_customer']) ? 'selected' : ''; ?>>-- Pilih Customer --</option>
            <?php
            mysqli_data_seek($customerResult, 0); // Reset pointer
            while ($row = mysqli_fetch_assoc($customerResult)) {
                $selected = (isset($_POST['id_customer']) && $_POST['id_customer'] == $row['id_customer']) ? 'selected' : '';
        ...
        <div class="form-group">
          ...
          <select name="id_pegawai" id="id_pegawai" class="form-control" required>
            <option value="" disabled <?php echo !isset($_POST['id_pegawai']) ? 'selected' : ''; ?>>-- Pilih Pegawai --</option>
            <?php
            mysqli_data_seek($pegawaiResult, 0); // Reset pointer
            while ($row = mysqli_fetch_assoc($pegawaiResult)) {
                $selected = (isset($_POST['id_pegawai']) && $_POST['id_pegawai'] == $row['id_pegawai']) ? 'selected' : '';
        ...
        <div class="form-group">
          ...
          <input type="date" name="tanggal" id="tanggal" class="form-control" value="<?php echo isset($_POST['tanggal']) ? htmlspecialchars($_POST['tanggal']) : ''; ?>" required>
        </div>
        <div class="form-group">
          <label for="DP">DP</label>
          <input type="number" name="DP" id="DP" class="form-control" value="<?php echo isset($_POST['DP']) ? htmlspecialchars($_POST['DP']) : '0'; ?>" required>
        </div>
        <div class="text-right">
          <a href="penjualan-lihat.php" class="btn btn-danger">Kembali</a>
