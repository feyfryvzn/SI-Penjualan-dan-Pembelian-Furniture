...
// Ambil semua customer & pegawai untuk dropdown
$customerList = mysqli_query($conn, "SELECT * FROM customer");
$pegawaiList = mysqli_query($conn, "SELECT * FROM pegawai");
...
if (isset($_POST['proses'])) {
    ...
    if (empty($id_customer) || empty($id_pegawai) || empty($tanggal)) {
        $error = "Semua kolom (kecuali No Nota) harus diisi!";
    } else {
        if ($conn) {
            $stmt = $conn->prepare("UPDATE penjualan SET id_customer = ?, id_pegawai = ?, tanggal = ? WHERE no_nota = ?");
            if ($stmt) {
                $stmt->bind_param("ssss", $id_customer, $id_pegawai, $tanggal, $no_nota);
                if ($stmt->execute()) {
                    header("Location: penjualan-lihat.php?success=update");
                    exit();
                } else {
...
<body>
  ...
  <div class="main">
    <div class="dashboard-header">
      <div>
        <h2 class="text-dark font-weight-bold mb-0">Ubah Penjualan</h2>
    ...
    <div class="card">
      <h5 class="mb-4 text-center">Form Ubah Penjualan</h5>
      <?php if (isset($error)): ?>
      ...
      <form action="" method="post">
        <div class="form-group">
          ...
          <input type="text" name="no_nota" id="no_nota" class="form-control" value="<?php echo htmlspecialchars($data['no_nota']); ?>" readonly>
        </div>
        <div class="form-group">
          ...
          <select name="id_customer" id="id_customer" class="form-control" required>
            ...
            <?php
            mysqli_data_seek($customerList, 0); 
            while ($row = mysqli_fetch_assoc($customerList)) {
                $selected = ($row['id_customer'] == $data['id_customer']) ? 'selected' : '';
        ...
        <div class="form-group">
          ...
          <select name="id_pegawai" id="id_pegawai" class="form-control" required>
            ...
            <?php
            mysqli_data_seek($pegawaiList, 0); 
            while ($row = mysqli_fetch_assoc($pegawaiList)) {
